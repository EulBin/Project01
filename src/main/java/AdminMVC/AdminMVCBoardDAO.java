...
public class AdminMVCBoardDAO extends DBConnPool{
	
	//부모클래스의 기본생성자 호출을 통해 DB를 연결한다.
	public AdminMVCBoardDAO() {
		super();
	}
	...
	*/
	public List<AdminMVCBoardDTO> selectListPage(Map<String, Object> map) {
        List<AdminMVCBoardDTO> board = new ArrayList<AdminMVCBoardDTO>();  
        ...
        try {
            ...
            while (rs.next()) {
                AdminMVCBoardDTO dto = new AdminMVCBoardDTO();
	...
	public int insertWrite(AdminMVCBoardDTO dto) {
    	//입력결과 확인용 변수
    	int result = 0;
	...
	public AdminMVCBoardDTO selectView(String idx) { 
        ...
        try {
            ...
            if (rs.next()) { 
                dto.setIdx(rs.getString(1)); 
                dto.setName(rs.getString(2)); 
                dto.setTitle(rs.getString(3));//인덱스를 통해 값 인출
                dto.setContent(rs.getString(4));//컬럼명을 통해 값 인출
                dto.setPostdate(rs.getDate(5));//날짜타입이므로 getDate()로 인출
                dto.setOfile(rs.getString(6));
                dto.setSfile(rs.getString(7));
	...
	//주어진 일련변호에 해당하는 게시물의 조회수를 1 증가시킨다.
	...
	public boolean confirmPassword(String pass, String idx) {
		...
		try{
			...
			없을때는 0, 만족하는 레코드가 있을때는 1 이상의 정수값이 반환된다.
	...
	//게시물 수정 처리
	public int updatePost(AdminMVCBoardDTO dto) {
		int result = 0;
		try {
