...
DAO(Data Access Object) : 실제 데이터베이스에 접근하여 여러가지
	CRUD작업을 하기위한 객체. 
*/
public class AdminMemberDAO extends JDBConnect {
	
	//인자가 4개인 부모의 생성자를 호출하여 DB에 연결한다. 
    public AdminMemberDAO(String drv, String url, String id, String pw) {
        super(drv, url, id, pw);
    }
    
    //application 내장객체만 매개변수로 전달하여 DB에 연결한다.
    public AdminMemberDAO(ServletContext application) {
        super(application);
    }
    ...
    */
    public AdminMemberDTO getMemberDTO(String uid, String upass) {
